...
jQuery.slideImage = function(imagepath, imagewidth, imageheight, rows) {
	...
	$original_image_src = $('#slidein').attr("src");

	// Create a container div around the original image
	$('#slidein').wrap('<div id="container" style="overflow: hidden; position: absolute; left: 0; width: ' + imagewidth + 'px; height: ' + imageheight + 'px"></div>');
	// Convert the original image to a div with the original image as the background
	$('#slidein').replaceWith('<div id="greyscale" style="position: absolute; left: 0; top: 0; width: ' + imagewidth + 'px; height: ' + imageheight + 'px; background-image: url(\'' + $original_image_src + '\'); background-repeat: no-repeat; background-position: center center; z-index: 1"></div>');
